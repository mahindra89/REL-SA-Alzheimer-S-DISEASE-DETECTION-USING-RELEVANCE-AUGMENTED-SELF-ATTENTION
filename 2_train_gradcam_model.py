"""
Training Script: Dual-Branch ResNet-18 with GradCAM
...
class GradCAM:
    ...
    def generate_cam(self, input_tensor, target_class):
        ...
        return torch.from_numpy(cam).float()

# ============================================================================
# 4. DUAL-BRANCH MODEL WITH GRADCAM
# ============================================================================

class DualBranchGradCAM(nn.Module):
    """
    Dual-Branch Architecture with GradCAM
    Branch 1: Original features (anatomical)
    Branch 2: GradCAM attention features
    """
...
model = DualBranchGradCAM(num_classes=CONFIG['num_classes']).to(CONFIG['device'])
...
print("\n" + "="*80)
print("🚀 STARTING TRAINING - Dual-Branch Model with GradCAM")
print("="*80)
print(f"Epochs: {CONFIG['num_epochs']}")
print(f"Batch Size: {CONFIG['batch_size']}")
print(f"Learning Rate: {CONFIG['learning_rate']}")
print(f"Architecture: Dual-Branch (Anatomical + GradCAM Attention)")
print("="*80 + "\n")
...
for epoch in range(CONFIG['num_epochs']):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), 
                   os.path.join(CONFIG['output_path'], 'gradcam_best_model.pth'))
...
print("\n" + "="*80)
print("✅ TRAINING COMPLETED - Dual-Branch Model with GradCAM")
print("="*80)
...
history_path = os.path.join(CONFIG['output_path'], 'gradcam_training_history.json')
...
axes[0].set_title('Dual-Branch GradCAM - Accuracy Progression')
axes[0].legend()
...
axes[1].set_title('Dual-Branch GradCAM - Loss Progression')
axes[1].legend()
axes[1].grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig(os.path.join(CONFIG['output_path'], 'gradcam_training_curves.png'), dpi=300, bbox_inches='tight')
print(f"✓ Training curves saved")

# Model summary
print("\n📋 Model Summary:")
print(f"   Architecture: Dual-Branch (Anatomical + GradCAM)")
print(f"   Attention Mechanism: Gradient-weighted Class Activation Maps")
